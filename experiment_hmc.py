...
# =====================================================================
# EXPERIMENT PIPELINE (POST-RUN DIAGNOSTICS)
# =====================================================================
def run_hmc_experiment(
    ...
    half_n = samples_np.shape[0] // 2            # R-hat
    ...
    B = half_n * ((mean1 - overall_mean)**2 + (mean2 - overall_mean)**2) 
    V_hat = ((half_n - 1) / half_n) * W + (1.0 / half_n) * B
    
    r_hat_values = np.sqrt(V_hat / (W + 1e-8)) 
    
    total_time = results['time']
    iters_per_sec = num_iterations / total_time
    
    print("\n" + "="*65)
    print(" HMC FINAL ESTIMATION RESULTS ")
    print("="*65)
    ...
    accepted_trace = results['accepted'].numpy().astype(float)
    ...
    ax_acc.axhline(0.65, color='gray', linestyle='--', linewidth=2, label='Ideal HMC (65%)')
